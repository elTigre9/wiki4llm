...
`wiki4llm run` in your terminal, and a CrewAI agent loop runs until every feature in
...
- **Python + CrewAI** — the Run Mode harness (`harness/`)
  - `crewai` for agent orchestration
  - `litellm` (bundled with crewai) for model-agnostic LLM calls
...
6. `05-agents.md` — the 5 CrewAI agents and their roles
7. `06-loop.md` — orchestration loop, idempotency, error handling
8. `07-vault-contract.md` — what each agent reads and writes
9. `08-llm-backends.md` — model strings, LiteLLM routing, per-agent config
